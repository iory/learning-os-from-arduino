...
int os_create_task(void (*entry)(void), const char *name, uint32_t stack_size)
{
    ...
    // 目印で埋めておく。あとで書き換わっていない範囲を数えれば、
    // そのタスクがスタックを何バイト使ったかが分かる。
    for (uint32_t i = 0; i < words; i++) {
        task->stack_base[-(int32_t)(i + 1)] = STACK_FILL_PATTERN;
    }
...
// 一度も書き換えられていないスタック領域のバイト数を返す。
// 0 に近いほど危険で、0 なら溢れている可能性が高い。
uint32_t os_get_stack_unused(int id)
{
    if (id < 0 || id >= g_task_count) {
        return 0;
    }
    ...
    uint32_t *low = task->stack_base - words;

    uint32_t i = 0;
    while (i < words && low[i] == STACK_FILL_PATTERN) {
        i++;
    }
    return i * 4;
}
